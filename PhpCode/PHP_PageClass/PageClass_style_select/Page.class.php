...
function page($Total_Size = 1, $Page_Size = 0, $Current_Page = 1, $listRows = 6, $PageParam = '', $PageLink = '', $Static = FALSE, $TP = "") {
    ...
    if (empty($TP)) {
        ...
        $TP = '<div style=" margin-top:30px;" class="yellow">{prev}{liststart}{list}{next}</div>';  //分页样式 $by zs1
    }
    $Page = new Page($Total_Size, $Page_Size, $Current_Page, $listRows, $PageParam, $PageLink, $Static);
    // $Page->SetPager('default', $TP, array("listlong" => "6", "first" => "首页", "last" => "尾页", "prev" => "上一页", "next" => "下一页", "list" => "*", "disabledclass" => ""));
    $Page->SetPager('default', $TP, array("listlong" => "6", "first" => "首页", "last" => "尾页", "prev" => "&lt", "next" => "&gt", "list" => "*", "disabledclass" => ""));     //分页样式修改 $by zs1
